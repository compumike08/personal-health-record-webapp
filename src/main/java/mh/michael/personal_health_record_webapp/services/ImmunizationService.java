...
public class ImmunizationService {
  ...
  @Transactional
  public List<ImmunizationDTO> getImmunizationsByPatientUuid(
    ...
    List<Immunization> immunizationList =
      immunizationRepository.findByPatient_PatientUuidOrderByImmunizationDateDesc(
        patientUuid
      );
  ...
  private Date validateImmunizationInputs(
    ...
    if (immunizationName.isEmpty()) {
      ...
      throw new ResponseStatusException(
        ...
        "You must specify a immunization name"
      );
    }
    ...
    if (immunizationDateString.isEmpty()) {
      ...
      throw new ResponseStatusException(
        HttpStatus.BAD_REQUEST,
        "You must specify an immunization date"
      );
    }
  ...
  public ImmunizationDTO deleteImmunization(
    String immunizationUuidString,
    JwtUserDetails jwtUserDetails
  ) {
    UUID currentUserUuid = jwtUserDetails.getUserUuid();
    UUID immunizationUuid = UUID.fromString(immunizationUuidString);

    Optional<Immunization> optImmunization =
      immunizationRepository.findByImmunizationUuid(immunizationUuid);

    if (optImmunization.isEmpty()) {
      log.error(
        "Unable to delete immunization as immunizationUuid {} not found",
        immunizationUuidString
      );
      throw new ResponseStatusException(
        HttpStatus.INTERNAL_SERVER_ERROR,
        INTERNAL_SERVER_ERROR_MSG
