...
public class MedicationService {
  ...
  @Transactional
  public List<MedicationDTO> getMedicationsByPatientUuid(
    ...
    List<Medication> medicationList =
      medicationRepository.findByPatient_PatientUuidOrderByIsCurrentlyTakingDesc(
        patientUuid
      );
  ...
  public MedicationDTO createMedication(
    ...
    Date medicationStartDate = GeneralUtil.parseDate(
      newMedicationRequestDTO.getMedicationStartDate(),
      null
    );
    Date medicationEndDate = GeneralUtil.parseDate(
      newMedicationRequestDTO.getMedicationEndDate(),
      null
    );
  ...
  public MedicationDTO deleteMedication(
    String medicationUuidString,
    JwtUserDetails jwtUserDetails
  ) {
    UUID currentUserUuid = jwtUserDetails.getUserUuid();
    UUID medicationUuid = UUID.fromString(medicationUuidString);

    Optional<Medication> optMedication = medicationRepository.findByMedicationUuid(
      medicationUuid
    );

    if (optMedication.isEmpty()) {
      log.error(
        "Unable to delete medication as medicationUuid {} not found",
        medicationUuidString
      );
      throw new ResponseStatusException(
        HttpStatus.INTERNAL_SERVER_ERROR,
        INTERNAL_SERVER_ERROR_MSG
  ...
  private void validateMedNameAndDosage(
    String medicationName,
    Double dosage,
    String dosageUnit
  ) {
    if (
      medicationName == null || medicationName.isEmpty() || medicationName.length() > 500
    ) {
      log.debug(
        "Validation Error: Medication name is null, empty, or greater than 500 characters long"
      );
      throw new ResponseStatusException(
    ...
    if (dosage != null && (dosageUnit == null || dosageUnit.isEmpty())) {
      log.debug(
        "Validation Error: Medication dosageUnit is null while dosage is not null"
      );
      throw new ResponseStatusException(
        HttpStatus.BAD_REQUEST,
  ...
  public MedicationDTO updateMedication(
    ...
    if (optMedication.isEmpty()) {
      log.error(
        "Unable to update medication as medicationUuid {} not found",
        medicationDTO.getMedicationUuid()
      );
      throw new ResponseStatusException(
        HttpStatus.INTERNAL_SERVER_ERROR,
        INTERNAL_SERVER_ERROR_MSG
    ...
    Date medicationStartDate = GeneralUtil.parseDate(
      medicationDTO.getMedicationStartDate(),
      null
    );
    Date medicationEndDate = GeneralUtil.parseDate(
      medicationDTO.getMedicationEndDate(),
      null
    );
