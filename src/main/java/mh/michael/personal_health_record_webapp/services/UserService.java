...
public class UserService {
  ...
  private void validateUsernameAndEmail(
    ...
    if (jwtUserDetails != null) {
      ...
      if (
        !jwtUserDetails.getEmail().equals(email) && userRepository.existsByEmail(email)
      ) {
  ...
  public UserDTO createNewUser(NewUserRequestDTO newUserRequestDTO) {
    ...
    Optional<UserRole> optUserUserRole = userRoleRepository.findByName(
      EUserRole.ROLE_USER
    );
  ...
  public UserDTO changeUserPassword(ResetPasswordRequestDTO resetPasswordRequestDTO) {
    String email = jwtTokenUtil.getSubjectFromToken(
      resetPasswordRequestDTO.getForgotPasswordToken()
    );
    ...
    if (
      !jwtTokenUtil.validateTokenForForgotPassword(
        resetPasswordRequestDTO.getForgotPasswordToken(),
        user
      )
    ) {
